...
# simpler time filter
def early_game_filter_simple(df, time_cutoff=1200000):
...
def feature_engineering(pklfile):
    ...
    dfearly = early_game_filter_simple(df,60000*20)
    ...
    # print(fullXdf.reset_index().set_index('matchId').shape)
    ...
    # Value_Feature.to_pickle('data_training/value_featuree.pkl')
    # Categorical_feature.to_pickle('data_training/categorical_featuree.pkl')
    
    ## create X variable
    X = np.dstack((Value_Feature,Categorical_feature)).astype(np.float32)
    X_flat = np.hstack((Value_Feature_flat,Categorical_feature_flat)).astype(np.float32)
...
if __name__ == '__main__':
    ...
    X,X_flat,Y,Y_flat = feature_engineering(pklfile) 
    with open('data_training/X.npy', 'wb') as f:
        np.save(f,X)
    with open('data_training/X_flat.npy', 'wb') as f:
        np.save(f,X_flat)
    with open('data_training/Y.npy', 'wb') as f:
        np.save(f,Y)
    with open('data_training/Y_flat.npy', 'wb') as f:
        np.save(f,Y_flat)
