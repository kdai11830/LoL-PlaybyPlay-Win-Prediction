...
def early_game_filter(df, time_cutoff=1200000):
    ...
    return df[(df['matchId'].isin(valid_matches)) & (df['sampleTimestamp'] <= time_cutoff)]

def dragon_handler(df):
    df['dragon_kill'] = df['OBJECTIVE_AIR_DRAGON']+df['OBJECTIVE_CHEMTECH_DRAGON']+df['OBJECTIVE_EARTH_DRAGON']+df['OBJECTIVE_FIRE_DRAGON']+df['OBJECTIVE_HEXTECH_DRAGON']
    Dragon_Kill = pd.DataFrame(df[['dragon_kill','matchId','sampleTimestamp','teamIdStr']].groupby(['matchId','sampleTimestamp'])['dragon_kill'].sum())
    Dragon_Kill = Dragon_Kill.rename(columns={'dragon_kill':'Total_dragon_kill'})
    df = df.merge(Dragon_Kill,on = (['matchId','sampleTimestamp']),how='left')
...
def feature_engineering(df,position_df):
    # dfearly = early_game_filter(df,60000*20,3)
    dfearly = early_game_filter(df,time_cutoff=60000*20)
    dfearly = dragon_handler(dfearly)

    sampleTS = dfearly[['sampleTimestamp']].sort_values(by='sampleTimestamp',ascending=True).drop_duplicates(keep='first')
    ## team 1 and team2
    ...
    position_df = early_game_filter(position_df, time_cutoff=60000*20)
    ...
    Value_Feature_flat = fullXdf.reset_index().set_index('matchId').to_numpy()
    print(Value_Feature_flat.shape)
    ...
    Categorical_feature_flat = Categorical_feature.reset_index().set_index('matchId').to_numpy()
    gb = Categorical_feature.groupby('matchId')
    Categorical_feature = np.array([gb.get_group(x).to_numpy() for x in gb.groups])
    print(Categorical_feature_flat.shape)
    ...
    Position_feature_flat = Position_feature.reset_index().set_index('matchId').to_numpy()
    ...
    print(Position_feature_flat.shape)
    
    ## create X variable
    X = np.dstack((Value_Feature,Categorical_feature,Position_feature)).astype(np.float32)
    X_flat = np.hstack((Value_Feature_flat,Categorical_feature_flat,Position_feature_flat)).astype(np.float32)
    ...
    YVariable_flat = fullXdf.reset_index().set_index('matchId')
    YVariable_flat = YVariable_flat.merge(T1_dfearly[['matchId','win']].groupby('matchId').first(),on='matchId',how='left')
    Y_flat = np.array(YVariable_flat['win']).astype(np.float32)

    matchorder = np.array(YVariable['matchId'])
    return X,X_flat,Y,Y_flat,matchorder
...
if __name__ == '__main__':
    ...
    for i in range(0,10):
        df = pd.read_pickle(f'aggregate_data/df_final_merge_final_{i}.pkl')
        df_flat = pd.read_pickle(f'aggregate_data/df_final_merge_flat_{i}.pkl')
        print(f'finish loading data chunk {i}')
        X,X_flat,Y,Y_flat,Order = feature_engineering(df,df_flat) 
        print(f'finish_feature_engineering chunk {i}')
        print(X.shape)
        print(X_flat.shape)
        with open(f'data_training_new/X_{i}.npy', 'wb') as f:
            np.save(f,X)
        with open(f'data_training_new/Y_{i}.npy', 'wb') as f:
            np.save(f,Y)
        with open(f'data_training_new/X_flat_{i}.npy', 'wb') as f:
            np.save(f,X_flat)
        with open(f'data_training_new/Y_flat_{i}.npy', 'wb') as f:
            np.save(f,Y_flat)
        with open(f'data_training_new/Order_{i}.npy', 'wb') as f:
            np.save(f,Order)   
    ...
    orderlists = []
    for i in range(0,10):
        with open(f'data_training_new/X_{i}.npy', 'rb') as f:
            Xlists.append(np.load(f))
        with open(f'data_training_new/Y_{i}.npy', 'rb') as f:
            Ylists.append(np.load(f))  
        with open(f'data_training_new/X_flat_{i}.npy', 'rb') as f:
            X_flatlists.append(np.load(f))
        with open(f'data_training_new/Y_flat_{i}.npy', 'rb') as f:
            Y_flatlists.append(np.load(f)) 
        with open(f'data_training_new/Order_{i}.npy', 'rb') as f:
            orderlists.append(np.load(f,allow_pickle=True))  
    X = np.vstack(Xlists)
    Y = np.concatenate(Ylists)
    X_flat = np.vstack(X_flatlists)
    Y_flat = np.concatenate(Y_flatlists)
    Order = np.concatenate(orderlists)

    print(X.shape)
    print(X_flat.shape)
    with open('data_training_new/X_final.npy', 'wb') as f:
        np.save(f,X)
    with open('data_training_new/Y_final.npy', 'wb') as f:
        np.save(f,Y)
    with open('data_training_new/X_flat_final.npy', 'wb') as f:
        np.save(f,X_flat)
    with open('data_training_new/Y_flat_final.npy', 'wb') as f:
        np.save(f,Y_flat)
    with open('data_training_new/Order_final.npy', 'wb') as f:
        np.save(f,Order)  
